...
# sql connector
try:
    connection = sqlite3.connect('sra.db', check_same_thread=False)
...
def run():
    ...
    lottie_ai = load_lottieurl("https://assets9.lottiefiles.com/packages/lf20_1rrxvx8h.json")
    if lottie_ai:
        st_lottie(lottie_ai, height=150, key="ai_animation")
        
    img = Image.open('./Logo/custom_logo_v2.jpg')
    ...
    if choice == 'User':
        ...
        ## Optional: Job Description for ATS matching
        jd_text = st.text_area("Paste Job Description for ATS Matching (Optional)", height=150)
        ...
        if pdf_file is not None:
            ...
            if resume_data:
                ...
                if jd_text:
                    ...
                    # Add parsed skills to resume words just in case
                    for skill in resume_data.get('skills', []):
                        resume_words.update(re.findall(r'\b[a-zA-Z]{3,}\b', skill.lower()))
                    ...
                    if total_jd_keywords > 0:
                        match_percentage = (match_count / total_jd_keywords) * 100
                    else:
                        match_percentage = 0
                        
                    st.markdown(f"**Your resume matches {match_percentage:.2f}% of the job description keywords.**")
                    
                    missing_keywords = jd_keywords - resume_words
                    if missing_keywords:
                        st.warning("Consider adding some of these keywords from the Job Description to improve your score:")
                ...
                lottie_skills = load_lottieurl("https://assets8.lottiefiles.com/packages/lf20_z01bika0.json")
                if lottie_skills:
                    st_lottie(lottie_skills, height=150, key="skills_animation")
                ...
                lottie_score = load_lottieurl("https://assets1.lottiefiles.com/packages/lf20_h9kds1my.json")
                if lottie_score:
                    st_lottie(lottie_score, height=150, key="score_animation")
    ...
    ###### CODE FOR ABOUT PAGE ######
        ...
        <p align="justify">
            Built with 🤍 by 
            <a href="#" style="text-decoration: none; color: #00e5ff;">Krishnakumar</a> through 
    ...
    ###### CODE FOR ADMIN SIDE (ADMIN) ######
        ...
        if st.button('Login'):
            ...
            if ad_user == 'admin' and ad_password == 'admin@resume-analyzer':
                ...
                st.success("Welcome Krishnakumar ! Total %d " % values + " User's Have Used Our Tool : )")                
